...
| `data.attributes.drift_detection`             | boolean | `false`   | Whether Terraform Cloud will perform [drift detection assessments](/cloud-docs/workspaces/settings/drift-detection) for the workspace. Only available for Business tier organizations, in workspaces running Terraform version 0.15.4+ and operating in [Remote execution mode](/cloud-docs/workspaces/settings#execution-mode).                                                                                                                                                                                |
...
A run will be triggered when a Git tag is pushed matching the regular expression (SemVer): `1.2.3`, `22.33.44`, etc.
